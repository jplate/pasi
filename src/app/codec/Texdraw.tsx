...
export const isFillable = (obj: any): obj is Fillable => typeof obj.fillLevel === 'number';
...
export const getStrokedShapes = (code: string, defaultLinewidth: number): StrokedShape[] => {
    ...
    while ((match = code.match(shapeFinder)) || lx.length > 0) {
        // Even after the shape commands run out, there may be a fill command to look out for,
        ...
        if (pathClosed || !match) {
            ...
            if (lxs > 0) {
                ...
                const ss = new StrokedShape(lx.length > 1 ? new Path([...lx], drawn, fillLevel) : s0, stroke);
                list.push(ss);
            }
        ...
        if (match) {
            ...
            switch (match[2]) {
                ...
                case '\\lcir':
                    if ((shapeMatch = shapeString.match(lfcirPattern))) {
                        shape = new Circle(
                            currentPoint,
                            decodeFloat(shapeMatch[2]),
...
export const decodeFloat = (s: string): number => {
    if (s === 'I') return Infinity;
        ...
        if (isNaN(result) || !s.match(floatPattern)) {
            // Since parseFloat is very lenient, we have to explicitly check whether s is a proper
...
export const getCommandSequence = (
    ...
    if (filled && filledShapes.length > 0) {
        if (!openPath && lw > 0) {
            // If there's no open path, we'll use lfill (provided that linewidth is not zero) and not add any commands for the drawnShapes.
            ...
            if (dash.length > 0) {
                // Since, in the texdraw code, we always reset the dash pattern after changing it to something other than the empty string, we
 ...
 */
export const getCommandSequenceForDrawnShapes = (
    shapes: Shape[],
    lw: number,
    dash: number[],
    prevLw?: number,
