...
export const IntroSection = ({ dark, keyCmd }: PasiSectionProps) => {
    return (
        <Section id='intro-section' dark={dark}>
            ...
            <p>
                To create a diagram, start by selecting one or (with {keyCmd('Shift')}-click)
    ...
    return (
        <Section id='keyboard-commands' header='Keyboard commands' dark={dark}>
            <p>
                ...
                area, the usual commands (including {keyCmd('Ctrl+Z')} and{' '}
                {isMac ? keyCmd('Shift+⌘+Z') : keyCmd('Ctrl+Y')} for &lsquo;undo&rsquo; and
                &lsquo;redo&rsquo;) are also available. To select a single node out of a
                group, hold {keyCmd('Ctrl')} pressed while clicking on it.
            </p>
    ...
    return (
        ...
        <ol>
            ...
            <li>
                ...
                {keyCmd('W')}, {keyCmd('A')}, {keyCmd('S')}, {keyCmd('D')}), it is possible to
