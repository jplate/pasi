...
const TransformTab = ({
    ...
    const originDescription = (
        <>
            <i>either</i>&thinsp; the last-selected location on the canvas <i>or</i>&thinsp; (if no location is selected) the center of either the currently
            focused or (if no node is focused) the last-selected node
        </>
    );
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <InputField
                ...
                lowTopMargin={true}
                onChange={(e) =>
                    setRotationIncrement((logIncrements.rotate = validInt(e.target.value, MIN_ROTATION_LOG_INCREMENT, MAX_ROTATION_LOG_INCREMENT)))
                }
            />
            <InputField
                label="Scale %"
                tooltip={
                    <>
                        Scale the selected nodes, using as origin {originDescription}. By default, scaling does not affect the sizes of the selected nodes but
                        only their distances to each other. This can be changed by selecting the relevant option below.
                    </>
                }
                tooltipPlacement="right"
                value={scaling}
            ...
            <InputField
                ...
                width={'short'}
                lowTopMargin={true}
                onChange={(e) => setScalingIncrement((logIncrements.scale = validInt(e.target.value, MIN_SCALING_LOG_INCREMENT, MAX_SCALING_LOG_INCREMENT)))}
            />
            <BasicColoredButton
                id="hflip-button"
                ...
                tooltip={
                    <>
                        Flips the current selection horizontally, using a vertical axis that runs through {originDescription}.<HotkeyComp mapKey="hflip" />
                    </>
                }
                tooltipPlacement="right"
            ...
            <BasicColoredButton
                ...
                tooltip={
                    <>
                        Flips the current selection vertically, using an horizontal axis that runs through {originDescription}.<HotkeyComp mapKey="vflip" />
                    </>
                }
                tooltipPlacement="right"
            ...
            />
            <CheckBoxField label="Scale entity nodes" value={scaleENodes} onChange={() => setScaleENodes((prev) => (transformFlags.scaleENodes = !prev))} />
            <CheckBoxField
                label="Scale linewidths"
                value={scaleLinewidths}
                onChange={() => setScaleLinewidths((prev) => (transformFlags.scaleLinewidths = !prev))}
            />
            <CheckBoxField label="Scale stroke patterns" value={scaleDash} onChange={() => setScaleDash((prev) => (transformFlags.scaleDash = !prev))} />
            <CheckBoxField
