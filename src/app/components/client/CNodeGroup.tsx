...
export default class CNodeGroup implements Group<CNode> {
    ...
    groupMove = (nodes: CNode[], dx: number, dy: number) => {
        ...
        nodes.forEach((node) => {
            ...
            dys[index] = dy;
            if (node.fixedAngles) {
                let current: CNode, x0, y0, cdx, cdy;
                for (const s of [-1, 1]) {
    ...
    equalizeCentralAngles = (node: CNode) => {
        ...
        do {
            // Adjusting angles can move the center. We'll repeat the process until the center stops moving by more than e.
            c = newC;
            for (let i = 0, angle = a0; i < n; i++, angle += dir * inc) {
                const j = i0 + i;
                const k = j >= n ? j - n : j;
    ...
    getInfoString(): string {
        ...
        let index = 0;
        if (this.linewidth > 0) {
            // If linewidth is zero, there will be no drawn lines, so index can be left at zero.
            let foundGap = false;
    ...
    parse(tex: string, info: string | null, dimRatio: number): void {
        ...
        const [omitLine, fixedAngles, activeMember] = arraysAsNum
            ? arraysFromNum
            : split.slice(1, firstNodeIndex).map((s) => {
                  let result: boolean[];
                  switch (s) {
                      case '*':
                          result = Array(n).fill(true);
                          break;
                      case '~':
        ...
        const curves = (
            fillLevel > 0 // Construct the array of curves from which we'll get information about the coordinates of our nodes:
                ? stShapes[0].shape instanceof Texdraw.Path
                    ? stShapes[0].shape.shapes
                    : [stShapes[0].shape]
                : stShapes.flatMap((sh) => (sh.shape instanceof Texdraw.Path ? sh.shape.shapes : [sh.shape]))
        ) as Texdraw.CubicCurve[];
        let curveIndex = 0,
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (fillLevel === 0) {
                /* If fillLevel is not zero, then all the curves should be present in the texdraw code (whether or not they are 
...
export const getLine = (node0: CNode, node1: CNode): CubicCurve => {
    ...
    return {
        x0: node0.x,
        y0: node0.y,
        x1: node0.x + Math.cos(a0) * node0.dist1,
        y1: node0.y + Math.sin(a0) * node0.dist1,
        x2: node1.x + Math.cos(a1) * node1.dist0,
        y2: node1.y + Math.sin(a1) * node1.dist0,
        x3: node1.x,
        y3: node1.y,
    };
};
