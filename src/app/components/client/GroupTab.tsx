    ...
    return (
        <div className='flex flex-col h-full'>
            <BasicColoredButton id='create-button' label='Create group' style='px-2 mx-2 mt-1 rounded-lg text-sm'
                disabled={false} 
                onClick={create} 
                tooltip={<>Creates a new group that contains all selected items (or, where applicable, the highest among their respective &lsquo;active&rsquo; {' '}
            ...
            <BasicColoredButton id='leave-button' label='Leave group' style='px-2 mx-2 mt-3 rounded-lg text-sm' 
                disabled={!canLeave} 
                onClick={leave} 
                tooltip={<>Deactivates the membership of each selected item (or of its second-highest active group, where applicable) in its currently highest active {' '}
                    group. Items will still be members of their respective groups, but selecting them will no longer select the other members {' '}
                    as well.<HotkeyComp mapKey='leave' /></>}
                tooltipPlacement='right' />
            <BasicColoredButton id='rejoin-button' label='Rejoin' style='px-2 mx-2 mt-2 rounded-lg text-sm' 
                disabled={!canRejoin} 
                onClick={rejoin} 
                tooltip={<>Reactivates the membership of the each selected item (or its highest active group, where applicable) in the next-lowest group. {' '}
                    <HotkeyComp mapKey='rejoin' /></>}
                tooltipPlacement='right' />
            <BasicColoredButton id='restore-button' label='Restore' style='px-2 mx-2 mt-2 rounded-lg text-sm' 
                disabled={!canRestore} 
                onClick={restore} 
                tooltip={<>Reactivates the membership of each member of each selected item&apos;s highest active group.<HotkeyComp mapKey='restore' /></>}
                tooltipPlacement='right' />
            <BasicColoredButton id='add-button' label='Add...' style='px-2 mx-2 mt-3.5 rounded-lg text-sm' 
                pressed={adding} 
                disabled={!canAdd} 
                onClick={changeAdding} 
                tooltip={<>Selecting items (while not holding <kbd>Ctrl</kbd> pressed) adds their highest active groups to the present group. Selecting items while holding {' '}
                    <kbd>Ctrl</kbd> pressed adds only the individual items.<HotkeyComp mapKey='adding' /></>}
                tooltipPlacement='right' />
            <BasicColoredButton id='dissolve-add-button' label='Dissolve-add...' style='px-2 mx-2 mt-2 rounded-lg text-sm' 
                ...
                tooltip={<>Selecting items adds either the members of their highest active groups (where applicable) or the items themselves to the present group. {' '}
                    Selecting items while holding <kbd>Ctrl</kbd> pressed adds only the individual items.<HotkeyComp mapKey='dissolve-adding' /></>}
                tooltipPlacement='right' />
        </div>
    )
}
