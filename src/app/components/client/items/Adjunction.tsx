...
export default class Adjunction extends SNode {
    ...
    arrowheadEditHandler: Handler = {
        ...
        hookLength: ({ e }: Info) => {
            if (e) {
                const d = parseInputValue(e.target.value, MINIMUM_HOOK_LENGTH, MAXIMUM_HOOK_LENGTH, this.hookLength, 
                    0, ROUNDING_DIGITS) - this.hookLength;
                return [(item, array) => {
    ...
    override parseArrowhead(stShapes: Texdraw.StrokedShape[], cpx: number, cpy: number, dimRatio: number, nodeName: string): Texdraw.StrokedShape[] {
        ...
        if (stShapes.length < 2) {
            throw new ParseError(<span>{complain(nodeName)}: expected two shapes for the arrowhead, but got only {stShapes.length}.</span>);
        }
        for (let i = 0; i < 2; i++) {
            const ss = stShapes[i];
            if (!(ss.shape instanceof Texdraw.Line)) {
                throw new ParseError(<span>{complain(nodeName)}: expected a line, but got {ss.shape.genericDescription}.</span>);
            }
        }
