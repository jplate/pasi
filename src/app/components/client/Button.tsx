...
export const BasicColoredButton = forwardRef(
    ({ id, label, icon, style, tooltip, tooltipPlacement, pressed, disabled, onClick }: ButtonProps, ref: React.ForwardedRef<HTMLButtonElement>) => {
        return (
            <BasicButton
                id={id}
                label={label}
                icon={icon}
                pressed={pressed}
                activeStyle="enabled:bg-btnactivebg enabled:text-btnactivecolor disabled:bg-btnbg disabled:text-btncolor"
                inactiveStyle="bg-btnbg/85 text-btncolor enabled:hover:bg-btnhoverbg"
                style={clsx('border-btnborder/50 enabled:hover:text-btnhovercolor', 'focus:ring-btnfocusring', style)}
                tooltip={tooltip}
