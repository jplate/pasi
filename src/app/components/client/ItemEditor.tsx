...
export const getRankMover = (val: string, selection: Item[]) => (item: Item, array: (ENode | CNodeGroup)[]) => {
    ...
    const selectedListMembers = selection
        .reduce((acc: (ENode | CNodeGroup)[], it) => (it instanceof ENode ? [...acc, it] : it instanceof CNode ? [...acc, it.group as CNodeGroup] : acc), [])
        ...
        <div className="flex flex-col h-full">
            {info.map((entry, i) => {
                switch (entry.type) {
                    ...
                    case 'textarea':
                        return <Textarea key={i} fullHeight={entry.fullHeight} value={entry.value} onChange={(e) => handleChange(e, entry.key ?? '')} />;
                    case 'menu':
                        return (
                            <MenuField
                                key={i}
